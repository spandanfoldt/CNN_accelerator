...
outputPath = r"D:\VLSI\Projects\NN_accelerator\NN_accelerator.sim\sim_1\behav\xsim"
headerPath = r"D:\VLSI\Projects\NN_accelerator\NN_accelerator.srcs\sources_1\new"
...
def genWaitAndBias(dataWidth,weightFracWidth,biasFracWidth,inputFile):
    ...
    weightHeaderFile = open(os.path.join(headerPath,"weightValues.h"),"w")
    ...
    for layer in range(0,len(myWeights)):
       for neuron in range(0,len(myWeights[layer])):
          ...
          f = open(os.path.join(outputPath,fi),'w')
    ...
    biasHeaderFile = open(os.path.join(headerPath,"biasValues.h"),"w")
